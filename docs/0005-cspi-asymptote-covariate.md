...
Status: ACCEPTED (signed off 2026-06-29; beta=1.5, clamp [0.60,1.60])
Date: 2026-06-27 (accepted 2026-06-29)
Relates to: #75 (Block 1+3 to production), #76 (blocked CSPI plot join), #91 (asymptote-covariate PR), ADR 0003 (hybrid recal engine)

## Sign-off (2026-06-29)

Approved at beta=1.5 with clamp [0.60,1.60], per the leave-one-ecoregion-out out-of-sample
assessment (20260628_cspi_deep_assessment.md): CSPI carries genuine, non-redundant productivity
signal for the cell asymptote (+8.4% held-out skill vs ClimateNA SI +0.9%, latitude +3.7%;
partial r +0.41 independent of SI), and CV skill rises monotonically with beta to the free slope,
so beta=1.5 is the skill/robustness balance. The structural finding stands: a uniform asymptote
scalar cancels in the t0-anchored reserve trajectory, so CSPI's effect is on the absolute/spatial
density layer (redistributive), not the anchored carbon trajectory. Per-forest-type calibration
tested and not adopted (only +1.3% held-out over global beta); documented caveat that a few large
eastern/southern types (Loblolly/shortleaf, Maple/beech/birch) have negative CSPI-asymptote slopes,
which the clamp bounds and a future targeted shrink can address.
