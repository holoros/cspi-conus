...
## ADDENDUM: per-forest-type slope calibration tested -- keep single global beta

Tested whether calibrating the CSPI->asymptote slope per forest type beats a single global beta
(leave-one-ecoregion-out CV, 1,801 cells, 29 forest types):

| Model | held-out RMSE | vs baseline |
|---|---|---|
| Global slope | 0.726 | +7.3% |
| Per-forest-type slope | 0.716 | +8.5% |

Per-ft beats global by only **+1.3%** despite per-ft slopes ranging widely (median 1.68, IQR
[0.78, 3.46], full range -0.85 to +5.75). The heterogeneity is largely noise out-of-sample (small-n
forest types have unstable slopes), so per-ft calibration is NOT worth the added complexity.
**Decision: keep the single global beta=1.5.**

**Important caveat surfaced:** several MAJOR forest types have negative or flat CSPI-asymptote
slopes -- Loblolly/shortleaf pine (-0.85, n=96), Maple/beech/birch (-0.40, n=108), Longleaf/slash
(+0.08), White/red/jack pine (+0.60) -- while productive conifer types are strongly positive
(Douglas-fir +5.75, Fir/spruce/mtn hemlock +5.09, Oak/hickory +4.03). So a uniform positive scalar
is directionally wrong for a handful of large eastern/southern types. The clamp limits the damage,
and the targeted future refinement (not now) is to zero or shrink CSPI scaling for the negative-slope
types specifically, rather than full per-ft calibration. For production, global beta=1.5 with the
clamp stands; this caveat is documented for the manuscript and for anyone tightening it later.

```
[PERFT]: per-ft adds only +1.3% held-out vs global -> keep single global beta=1.5. Caveat: Loblolly/shortleaf and Maple/beech/birch have negative CSPI slopes (uniform positive scalar mis-signs them; clamp limits damage; targeted zero/shrink is the future fix).
```
